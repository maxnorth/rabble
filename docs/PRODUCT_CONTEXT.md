...
2. **Service vs. user auth, visible everywhere.** Whose credential a tool call carries is a property of the **MCP server registration's credential mode**, chosen once when it is registered: **shared** (one org credential; calls run as the org **service** account) or **personal** (each caller connects their own account under Profile; calls run **as the user**, with a consent prompt, and pause on an in-session connect card when no account is connected yet). The same upstream server may be registered twice to expose some tools shared and others personal. Consent and identity are surfaced at every level: server-level mode, inline session approval, analytics, personal connections. (Superseded the earlier per-tool service/user selector, which labelled consent the credential layer could not honor.)
